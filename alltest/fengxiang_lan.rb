...
class CS253EnumTests < Minitest::Test
	...
	str_each_entry.cs253each_entry{|e| str_arr << e.length}
	arr_each_entry.cs253each_entry{|e| arr_arr << e}
	triple.cs253each_entry{|i| trp_arr << (i<3)}

    	assert_equal int_arr.to_a, [10,20,30]
    	assert_equal str_arr.to_a, [3,3,4]
    	assert_equal arr_arr.to_a,[[1,2,3],[4,5,6],[7,8,9]]
	...
	triple = Triple.new('abcd','abdc','acdb')
	...
	assert_equal triple.cs253grep(/cb/){ |v| v}.to_a, []
    end
	...
	triple = Triple.new("apple", "application", "banana")
	...
	assert_equal triple.cs253grep_v(/app/){|str| str[0]}.to_a, ["b"]
    end
	...
	assert_equal triple.cs253inject{|str1, str2| str1 + " " + str2}.to_s, "apple is fruit"
    end
	...
	assert_equal triple.cs253max(1) {|a, b| a.length <=> b.length }.to_a, ["albatross"]
    end
	...
	assert_equal triple.cs253min(1) {|a, b| a.length <=> b.length }.to_a, ["dog"]
    end
	...
	assert_equal triple.cs253reduce{|product, n| product * n}.to_i, 30
    end
	...
	assert_equal triple.cs253reverse_each{|i| i}.to_a, ["duck", "dog", "cat"]
    end
    ...
    def test_to_a
	arr_to_a_one = CS253Array.new([1,2,3])
    	arr_to_a = CS253Array.new([[1,2,3],[4,5,6],(1..6)])
	triple = Triple.new('cat','dog','duck')
	
    	assert_equal arr_to_a_one.cs253to_a, [1, 2, 3]
	...
	arr_to_h = CS253Array.new([[1,2],[3,4],[5,6]])
    	arr_to_h_two = CS253Array.new([['animal','cat'],['fruit','apple'], ['animal','dog']])
	triple = Triple.new(['animal','cat'],['fruit','apple'], ['animal','dog'])	

    	assert_equal arr_to_h.cs253to_h, {1=>2,3=>4,5=>6}
    	assert_equal arr_to_h_two.cs253to_h,{"animal"=>"dog", "fruit"=>"apple"}
	...
	arr_one_zip.cs253zip(triple){|x,y| zip_four << [y, x]}
	...
	assert_equal zip_four.to_a, [["cat", 1], ["dog", 2], ["cat", 3]]
    end
